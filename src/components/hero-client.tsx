    ...
    <section className="relative mb-8 overflow-hidden sm:mb-12 md:mb-[60px] lg:mb-[70px]">
      ...
      {backgroundType === "video" && backgroundVideo ? (
        <>
          ...
          {/* Overlay for text readability. Account 137 uses a white wash so the
              hero reads light with dark text; everyone else gets a dark overlay. */}
          <div
            className={`absolute inset-0 -z-10 ${account137 ? "bg-white/40" : "bg-black/40"}`}
          />
        </>
      ) : backgroundImage ? (
        <>
